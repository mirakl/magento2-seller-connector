...
class MassNewOffer extends AbstractListing
{
    /**
     * @var Filter
     */
    ...
    public function execute()
    {
        ...
        try {
            ...
            $offerResource = $this->_objectManager->create(OfferResource::class);
            $offerResource->markOffersAsNew($listing->getId(), $productIds);
            $offerResource->markProductsAsNew($listing->getId(), $productIds);

            $messages[] = [
                'type' => 'success',
                'message' => __('Selected prices & stocks will be exported with product info during the next export.')
            ];
            $refresh = true;
        } catch (\Exception $e) {
            $messages[] = [
